...
test('analisa os dois mapas reais com 27 entradas', () => {
...
test('edita somente a entrada e os campos solicitados', () => {
  ...
  const result = editArtSource(covers, 'cover', 'sorcerer', { overlay: .72 });
  const after = parseArtSource(result.source, 'covers').entries;
  assert.equal(after.sorcerer.overlay, .72);
  ...
  const changedBytes = result.source.replace('overlay: 0.72', 'overlay: 0.80');
  assert.equal(changedBytes, covers);
});

test('insere scale estruturalmente quando ausente', () => {
...
test('suporta cover, detail art e ambos sem alterar os arquivos reais', () => {
  assert.equal(editArtSource(covers, 'cover', 'wizard', { blur: 4 }).after.blur, 4);
  assert.equal(editArtSource(details, 'detailArt', 'wizard', { overlay: .55, scale: 1.1 }).after.scale, 1.1);
});

test('rejeita ID ausente, sintaxe malformada e propriedades duplicadas', () => {
  assert.throws(() => editArtSource(covers, 'cover', 'missing', { overlay: .5 }), /não possui entrada/i);
