...
check(manifest.version==='5.63.0','manifest sincronizado em 5.63.0');
check(manifest.raceBuildPhase==='RB-4'&&manifest.raceBuildEligibilityIntegrated===true&&manifest.raceBuildFoundryExportEnabled===true&&manifest.raceBuildExportUIIntegrated===true,'manifest declara RB-4 + eligibility/export UI integrados');
...
check(R?.phase==='RB-3','resolver preserva eligibility engine RB-3 sob a UI RB-4');
...
check(human.capabilities.exportEnabled===true,'RB-4 expõe capacidade de exportação quando canExport=true');
...
check(bundle.readiness.canExport===human.canExport&&bundle.readiness.exportEnabled===true,'bundle RB-4 habilita exportEnabled quando canExport=true');
check(bundle.foundryPlan.status==='awaiting-importer','bundle RB-4 permanece awaiting-importer até RB-5');
...
check(read('js/race-browser.js').includes('Exportar para Foundry'),'RB-4 adiciona botão funcional de exportação ao Race Browser');


// Fixtures RB-4 para o futuro preflight do Importer.
for(const fixture of ['examples/races/human-woodlander-rb4.json','examples/races/hanyou-emberash-rb4.json']){
  const parsed=JSON.parse(read(fixture));
  check(B.inspectBundle(parsed).ok&&parsed.readiness.exportEnabled===true,`fixture ${fixture} permanece válida/exportável`);
}

// RB-4: contrato exportável, UI de prévia e JSON determinístico.
const uiSource=read('js/exporters/foundry-race-export-ui.js');
check(uiSource.includes('Race Build Bundle v1')&&uiSource.includes('downloadFoundryRaceJson')&&uiSource.includes('copyFoundryRaceJson'),'RB-4 possui UI própria de prévia/cópia/download JSON');
check(uiSource.includes('Grimório Importer 0.12.0')&&uiSource.includes('RB-5'),'UI deixa explícito que o Importer está congelado e o consumo começa em RB-5');
const readyBundle=B.buildBundle(human);
check(B.inspectBundle(readyBundle).ok&&readyBundle.readiness.exportEnabled===true,'bundle elegível passa validação final e exportEnabled=true');
const notReady=B.buildBundle('human',{});
check(B.inspectBundle(notReady).ok&&notReady.readiness.canExport===false&&notReady.readiness.exportEnabled===false,'bundle incompleto continua estruturalmente válido, mas não exportável');
check(JSON.parse(B.stringify(readyBundle)).identity.grimorioId===readyBundle.identity.grimorioId,'JSON RB-4 serializa o bundle sem perda de identidade');

// Smoke da UI RB-4 sem DOM real: buildRaceState/renderState devem usar o mesmo resolver e bundle.
const ui4=ctx({
  GRIMORIO_RACE_BROWSER:{
    getRace:id=>R.findRace(id),
    getBuildState:id=>id==='human'?human.state:{},
    resolveBuild:(id,st)=>R.resolve(id,st)
  },
  GRIMORIO_FOUNDRY_RACE_BUILD_BUNDLE:B,
  document:undefined
});
load(ui4,'js/exporters/foundry-race-export-ui.js');
const uiReady=ui4.GRIMORIO_FOUNDRY_RACE_EXPORT_UI.buildRaceState('human');
check(uiReady.ok&&uiReady.json.includes('grimorio-foundry-race-build-bundle')&&uiReady.filename.endsWith('-foundry.json'),'UI RB-4 gera JSON/filename para build elegível');
check(ui4.GRIMORIO_FOUNDRY_RACE_EXPORT_UI.renderState(uiReady).includes('Inspecionar JSON que será exportado'),'prévia RB-4 exibe inspeção do JSON');

// Race Browser deve renderizar botão habilitado/desabilitado de acordo com canExport.
const readyStorage=new Map([['grimorio-race-builder-v2',JSON.stringify({schema:'grimorio-race-builder-ui',schemaVersion:2,search:'',filter:'all',byRace:{human:human.state}})]]);
const rbReady=ctx({localStorage:{getItem:k=>readyStorage.get(k)||null,setItem:(k,v)=>readyStorage.set(k,String(v)),removeItem:k=>readyStorage.delete(k)},routeHref:(...a)=>'#/'+a.join('/'),shareLinkButton:()=>'',navigate:()=>{},render:()=>{},document:{querySelectorAll:()=>[],getElementById:()=>null}});
for(const f of FILES.slice(0,14))load(rbReady,f);load(rbReady,'js/race-browser.js');
const readyHtml=rbReady.GRIMORIO_RACE_BROWSER.renderRace('human');
check(/Exportar para Foundry<\/button>/.test(readyHtml)&&!/Exportar para Foundry<\/button>/.test(readyHtml.replace(/<button[^>]*disabled[^>]*>[^<]*(?:<[^>]+>[^<]*)* Exportar para Foundry<\/button>/,'')),'Race Browser exibe ação RB-4 para build pronto');
const emptyStorage=new Map();const rbEmpty=ctx({localStorage:{getItem:k=>emptyStorage.get(k)||null,setItem:(k,v)=>emptyStorage.set(k,String(v)),removeItem:k=>emptyStorage.delete(k)},routeHref:(...a)=>'#/'+a.join('/'),shareLinkButton:()=>'',navigate:()=>{},render:()=>{},document:{querySelectorAll:()=>[],getElementById:()=>null}});
for(const f of FILES.slice(0,14))load(rbEmpty,f);load(rbEmpty,'js/race-browser.js');
const blockedHtml=rbEmpty.GRIMORIO_RACE_BROWSER.renderRace('human');
check(blockedHtml.includes('Exportar para Foundry')&&/disabled title=/.test(blockedHtml),'Race Browser mantém botão RB-4 desabilitado quando faltam escolhas');

// Ordem de scripts e freeze do Importer.
const index=read('index.html');const rulesAt=index.indexOf('data/race-build-eligibility-rules.js'),resolverAt=index.indexOf('js/race-build-resolver.js'),bundleAt=index.indexOf('js/exporters/foundry-race-build-bundle.js'),browserAt=index.indexOf('js/race-browser.js'),raceUiAt=index.indexOf('js/exporters/foundry-race-export-ui.js');
check(rulesAt>=0&&resolverAt>rulesAt&&bundleAt>resolverAt&&browserAt>bundleAt&&raceUiAt>browserAt,'index carrega eligibility → resolver → bundle → Race Browser → UI RB-4');
...
finish();
function finish(){for(const m of passed)console.log('✓ '+m);if(errors.length){console.error(`\nRace Build RB-4 reprovado: ${errors.length} erro(s).`);for(const m of errors)console.error('✗ '+m);process.exit(1)}console.log(`\nRace Build RB-4 v5.63.0 aprovado: ${passed.length} verificações, 0 erros.`)}
