...
const pkg = api.buildCatalogPackage('lyre-retia-feats');
const validation = api.validatePackage(pkg);
if (!validation.ok) {
  console.error(validation.errors.join('\n'));
  process.exit(1);
}
const out = path.join(root, 'foundry/grimorio-importer/examples/feats/lyre-retia-feats-package-v2.json');
fs.writeFileSync(out, api.stringify(pkg), 'utf8');
console.log(`✓ ${pkg.summary.feats} talentos de Lyre exportados para ${path.relative(root, out)}`);
