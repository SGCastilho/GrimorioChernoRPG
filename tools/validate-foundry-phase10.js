...
(async () => {
  ...
  assert(['5.22.1','5.23.0','5.24.0','5.25.0','5.25.1'].includes(manifest.version), 'manifest.json deve preservar a integração granular da Fase 10');
  assert(['0.6.0','0.7.0','0.8.0','0.9.0','0.9.1'].includes(moduleManifest.version), 'Grimório Importer deve preservar suporte da Fase 10');
  ...
  assert(classCount === 24, `Esperadas 24 classes; encontradas ${classCount}`);
  assert(subclassCount === 377, `Esperadas 377 subclasses; encontradas ${subclassCount}`);
  ...
  // Estado/JSON real do exportador: todos os 401 itens devem gerar arquivos individuais válidos.
  ...
  assert(fullCatalog.ok && fullCatalog.summary.bundles === 401, 'Catálogo completo deve manter 401 bundles');
