...
const catalog = reg?.getFeatCatalogs?.().find(item => item.id === 'ryoko-yokai-realms-feats');
if (!catalog) fail('Catálogo ryoko-yokai-realms-feats ausente.');
else {
  if (catalog.sourceId !== 'ryoko-yokai-realms') fail(`sourceId incorreto: ${catalog.sourceId}`);
  if (catalog.pages !== '216–223') fail(`Faixa de páginas incorreta: ${catalog.pages}`);
  if (catalog.feats.length !== 39) fail(`Esperados 39 talentos de Ryoko; encontrados ${catalog.feats.length}.`);
  const ids = new Set(catalog.feats.map(item => item.id));
  if (ids.size !== catalog.feats.length) fail('Há IDs duplicados nos talentos de Ryoko.');
  const general = catalog.feats.filter(item => item.category === 'Geral');
  const racial = catalog.feats.filter(item => item.category === 'Racial');
  if (general.length !== 26) fail(`Esperados 26 talentos gerais; encontrados ${general.length}.`);
  if (racial.length !== 13) fail(`Esperados 13 talentos raciais; encontrados ${racial.length}.`);
  ...
  if (prerequisites !== 22) fail(`Esperados 22 talentos de Ryoko com pré-requisito; encontrados ${prerequisites}.`);
  if (repeatable !== 2) fail(`Esperados 2 talentos de Ryoko repetíveis; encontrados ${repeatable}.`);
  for (const feat of catalog.feats) {
    ...
    if (!Number.isInteger(feat.sourcePage) || feat.sourcePage < 216 || feat.sourcePage > 223) fail(`${feat.id}: sourcePage inválida (${feat.sourcePage}).`);
...
if (catalog && automation) {
  ...
  ok('39 perfis de Ryoko validados em tier description, sem automação inferida');
}

const pkgApi = context.GRIMORIO_FOUNDRY_FEAT_PACKAGE;
let generated;
if (pkgApi) {
  generated = pkgApi.buildCatalogPackage('ryoko-yokai-realms-feats');
  const validation = pkgApi.validatePackage(generated);
  if (!validation.ok) validation.errors.forEach(error => fail(`Exporter: ${error}`));
  if (generated.summary.feats !== 39 || generated.summary.automation?.description !== 39) fail('Resumo do pacote Ryoko não fecha em 39/39 description.');
  ok('Feat Package v2 de Ryoko validado pelo exportador do site');
}
...
const fixture = path.join(root, 'foundry/grimorio-importer/examples/feats/ryoko-yokai-realms-feats-package-v2.json');
if (!fs.existsSync(fixture)) fail('Fixture Foundry de Ryoko ausente.');
else if (generated) {
  const saved = JSON.parse(fs.readFileSync(fixture, 'utf8'));
  if (JSON.stringify(saved) !== JSON.stringify(generated)) fail('Fixture Foundry de Ryoko está dessicronizado com o exportador atual.');
  else ok('Fixture Foundry de Ryoko sincronizado com o exportador');
}
...
console.log('\nValidação Ryoko 5.43 aprovada sem falhas.');
