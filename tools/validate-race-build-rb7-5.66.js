...
check(pkg.version === '5.66.0' && lock.version === '5.66.0' && lock.packages?.['']?.version === '5.66.0' && manifest.version === '5.66.0' && read('js/config.js').includes("APP_VERSION='5.66.0'"), 'versão Grimório 5.66.0 sincronizada');
check(mod.version === '0.13.0-alpha.3' && ipkg.version === '0.13.0-alpha.3' && versionJs.includes('0.13.0-alpha.3') && versionJs.includes('phase: "RB-7"'), 'Importer 0.13.0-alpha.3 / RB-7 sincronizado');
check(manifest.raceBuildPhase === 'RB-7' && manifest.raceBuildMaterializationIntegrated === true && manifest.raceBuildAutomationIntegrated === true && manifest.raceBuildActorApplication === false, 'manifesto declara automação RB-7 sem aplicação em Actor');
check(manifest.foundryExport?.raceBuild?.phase === 'RB-7' && manifest.foundryExport?.raceBuild?.materialization === true && manifest.foundryExport?.raceBuild?.automation === true && manifest.foundryExport?.raceBuild?.actorApplication === false, 'foundryExport.raceBuild descreve a fronteira RB-7');
...
check(siteExporter.includes("status: 'materialization-supported'") && siteExporter.includes("importerStatus: 'rb7-automation'") && siteExporter.includes("phase: 'RB-7'"), 'site exporta materialization-supported com consumidor RB-7');
...
check(siteUi.includes('RB-7 · Race Build Bundle v1') && siteUi.includes('0.13.0-alpha.3'), 'UI de exportação anuncia o consumidor RB-7 atual');
...
check(raceValidator.includes('phase: "RB-7"') && raceValidator.includes('materializa e automatiza com política conservadora'), 'validator racial reporta fase RB-7');
const support = read('foundry/grimorio-importer/scripts/race-support.js');
check(support.includes('phase: "RB-7"') && support.includes('nextPhase: "RB-8"'), 'support racial aponta RB-7 → RB-8');
...
check(preflight.includes('plannedRaceBuildDocuments') && preflight.includes('phase: "RB-7"') && preflight.includes('raceBuildPreflightOnly: false'), 'preflight continua executável e alinhado à RB-7');
const readiness = read('foundry/grimorio-importer/scripts/ui/release-readiness.js');
check(readiness.includes('race-automation') && readiness.includes('0.13.0-alpha.3') && readiness.includes('phase) === "RB-7"'), 'gate de readiness exige a automação RB-7');
...
check(main.includes('raceAutomationSupport') && main.includes('materializeRaceBuild') && main.includes('importRaceBuild'), 'API principal expõe support RB-7 e mantém materialização racial');
...
check(human7.foundryPlan?.status === 'materialization-supported' && /RB-7/.test(human7.foundryPlan?.note ?? '') && hanyou7.foundryPlan?.status === 'materialization-supported', 'fixtures atuais usam materialization-supported + nota RB-7');
...
run('foundry/grimorio-importer/tests/validate-race-preflight-013.mjs', 'gate de preflight racial RB-7 aprovado');

if (errors.length) {
  console.error(`Race Build RB-7 v5.66.0 reprovado: ${errors.length} erro(s).`);
  errors.forEach(e => console.error('✗ ' + e));
  process.exit(1);
}
passed.forEach(m => console.log('✓ ' + m));
console.log(`\nRace Build RB-7 v5.66.0 aprovado: ${passed.length} verificações, 0 erros.`);
