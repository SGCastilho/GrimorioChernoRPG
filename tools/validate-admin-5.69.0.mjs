...
check(manifest.version === '5.69.0' && packageJson.version === '5.69.0' && packageLock.version === '5.69.0' && read('js/config.js').includes("APP_VERSION='5.69.0'"), 'versão 5.69.0 sincronizada');
...
if (errors.length) {
  ...
  console.error(`Admin 5.69.0 reprovado: ${errors.length} erro(s).`);
  process.exit(1);
}
console.log(`Admin 5.69.0 aprovado: ${passed.length} verificações, 0 erros, 0 avisos.`);
