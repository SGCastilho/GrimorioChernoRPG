...
check(manifest.version === '5.70.0' && packageJson.version === '5.70.0' && packageLock.version === '5.70.0' && read('js/config.js').includes("APP_VERSION='5.70.0'"), 'versão 5.70.0 sincronizada');
check(packageJson.dependencies?.acorn === '8.18.0', 'Acorn 8.18.0 fixado como dependência de produção');
check(Array.isArray(manifest.classIndex) && manifest.classIndex.length === manifest.classes && manifest.classes === 28, 'manifest.classIndex preserva as 28 classes reais');
...
check(Object.keys(covers).length === 28 && Object.keys(details).length === 28, 'mapas de classe preservam 28 entradas');
...
if (errors.length) {
  ...
  console.error(`Admin 5.70.0 reprovado: ${errors.length} erro(s).`);
  process.exit(1);
}
console.log(`Admin 5.70.0 aprovado: ${passed.length} verificações, 0 erros, 0 avisos.`);
