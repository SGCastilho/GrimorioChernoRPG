...
check(manifest.version === '5.57.0' && packageJson.version === '5.57.0' && packageLock.version === '5.57.0' && read('js/config.js').includes("APP_VERSION='5.57.0'"), 'versão 5.57.0 sincronizada');
...
check(serverSource.includes('SPELL_CATALOG_FILES') && serverSource.includes("'data/phb-spells.js'") && serverSource.includes("'data/homebrew-sage-spells.js'"), 'allowlist fixa cobre os dez catálogos reais de magias');
...
check(publicSource.includes("adminRequest('race')") && publicSource.includes('46 raças e 368 subraças') && publicSource.includes('originalName protegido'), 'interface racial usa catálogo real, preview e sinaliza campos protegidos');
check(publicSource.includes("adminRequest('spell')") && publicSource.includes('1.185 registros') && publicSource.includes('aliases, versões históricas'), 'interface de magias usa índice real, preview e sinaliza estruturas protegidas');
...
if (errors.length) {
  ...
  console.error(`Admin 5.57.0 reprovado: ${errors.length} erro(s).`);
  process.exit(1);
}
console.log(`Admin 5.57.0 aprovado: ${passed.length} verificações, 0 erros, 0 avisos.`);
