  ...
  if (bundle.kind === "class") {
    ...
    if (bundle.nativeMapping?.status !== "ready" && !SPECIAL_CLASS_ALLOWLIST.has(identifier)) {
      if (READY_CLASS_ALLOWLIST.has(identifier)) {
        warnings.push(`O bundle sinaliza mapeamento nativo em revisão, mas o Grimório Importer ${IMPORTER_VERSION} possui perfil local homologado para ${bundle.identity?.name ?? identifier}. O perfil local será usado.`);
      } else {
