...
(function initFeatBrowser(global) {
  ...
  function searchText(feat) {
    ...
    return normalize([
      feat.name, feat.description, feat.prerequisite, sourceLabel(feat), feat.sourcePage,
  ...
  function featCard(feat, targetId=null) {
    ...
    return '<details class="feat-item" data-feat-id="'+attr(feat.id)+'" '+(targetId===feat.id?'open data-route-target="true"':'')+'>'
      +'<summary class="feat-summary"><div class="feat-icon" aria-hidden="true">✦</div><div class="feat-title-block"><strong><a href="'+attr(href)+'" data-grimorio-route onclick="event.stopPropagation()">'+esc(feat.name)+'</a></strong><span>'+esc(sourceLabel(feat))+' · p. '+esc(feat.sourcePage)+'</span><a class="feat-deep-link" href="'+attr(href)+'" data-grimorio-route onclick="event.stopPropagation()">Abrir em link direto ↗</a></div>'
