...
(function(){
  ...
  function raceState(id){
    ...
    const limit=legacyLimit(r);
    return {subraceId:validSub,legacy:Array.isArray(raw.legacy)?raw.legacy.slice(0,limit):[],mixed:!!raw.mixed,secondaryRaceId:secondary};
  }
  function legacyLimit(race){return Math.max(2,Number(race?.legacyChoices)||2)+(Math.max(0,Number(race?.dominantLegacyBonus)||0));}
  function legacyIsBonusEligible(race,key){return String(key||'').startsWith(race.id+':legacy:');}
  function canSelectLegacy(race,current,key){const candidate=[...current,key],base=Math.max(2,Number(race?.legacyChoices)||2),bonus=Math.max(0,Number(race?.dominantLegacyBonus)||0);if(candidate.length>base+bonus)return false;if(!bonus)return candidate.length<=base;const nonBonus=candidate.filter(k=>!legacyIsBonusEligible(race,k)).length;return nonBonus<=base;}
  ...
  function toggleLegacy(raceId,key){
    ...
    const i=rs.legacy.indexOf(key);if(i>=0)rs.legacy.splice(i,1);else if(canSelectLegacy(r,rs.legacy,key))rs.legacy.push(key);setRaceState(raceId,rs);refresh();
  }
  ...
  function catalogInner(){
    ...
    return '<div class="race-hero"><div><div class="eyebrow"><span class="dot"></span>Opções de personagem — 5.19</div><h1 class="page-title">Raças e Subraças</h1><p class="lede">Consulte os traços raciais fixos, compare subraças e monte suas duas escolhas de <b>Traços de Legado</b> em um único lugar. O conteúdo desta ferramenta reúne o capítulo de Opções de Personagem de Lyre e expansões raciais compatíveis da 5.19, preservando a fonte própria de cada subraça.</p></div><div class="race-hero-stats"><div><strong>'+races().length+'</strong><span>raças</span></div><div><strong>'+subCount+'</strong><span>subraças</span></div><div><strong>2</strong><span>Traços de Legado</span></div></div></div>'
      ...
      +'</div></section>'
      +(rr.paraprismaticTempest&&Array.isArray(rr.paraprismaticTempest.planarLegacyTraits)?'<section class="race-detail-section"><div class="race-section-title"><div><span>Paraprismatic Tempest · opcional</span><h2>Traços de Legado Planares</h2></div><span class="race-source-chip">Fonte p. 37</span></div><p class="race-section-intro">Com autorização do Mestre, personagens tocados pelos Planos Elementais podem tratar estas opções como parte da lista de Traços de Legado de sua raça.</p><div class="race-trait-grid">'+rr.paraprismaticTempest.planarLegacyTraits.map(t=>traitCard(t)).join('')+'</div></section>':'')
  ...
  function selectedSubraceTraits(race,sub){
    ...
    if(race.id==='hanyou'){
      const ruleCards=sub.traits.filter(t=>t.heritageRole==='rule');
      ...
      const hr={...(race.heritageRules||{}),...(sub.heritageRules||{})};
      const positiveChoices=Number(hr.positiveChoices)||2,detrimentalChoices=Number(hr.detrimentalChoices)||2;
      ...
      extra+='<div class="hanyou-heritage-note"><strong>Escolhas de Herança Hanyou</strong><span>Como Hanyou dominante, escolha '+esc(positiveChoices)+' Traço'+(positiveChoices===1?'':'s')+' de Herança positivo'+(positiveChoices===1?'':'s')+' e '+esc(detrimentalChoices)+(detrimentalChoices===1?' prejudicial':' prejudiciais')+'.'+sourceSpecific+' Se Hanyou for sua raça secundária, aplicam-se as regras de Sangue Misto e Linhagem descritas para Hanyou.</span></div>';
      if(ruleCards.length)extra+='<div class="heritage-group rule"><h4>Regras da Herança</h4><div class="race-trait-grid">'+ruleCards.map(t=>traitCard(t)).join('')+'</div></div>';
      if(lineage.length)extra+='<div class="heritage-group lineage"><h4>Traço'+(lineage.length===1?'':'s')+' da linhagem</h4><div class="race-trait-grid">'+lineage.map(t=>traitCard(t)).join('')+'</div></div>';
      if(positive.length)extra+='<div class="heritage-group positive"><h4>Traços de Herança positivos — escolha '+esc(positiveChoices)+'</h4><div class="race-trait-grid">'+positive.map(t=>traitCard(t)).join('')+'</div></div>';
      if(detrimental.length)extra+='<div class="heritage-group detrimental"><h4>Traços de Herança prejudiciais — escolha '+esc(detrimentalChoices)+'</h4><div class="race-trait-grid">'+detrimental.map(t=>traitCard(t)).join('')+'</div></div>';
      return extra;
    }
  ...
  function renderRace(id,forcedSubraceId=null){
    ...
    const full=isFullRace(race),legacyMax=legacyLimit(race);
    ...
    return '<div class="race-detail '+(full?'full-text':'')+'"><div class="race-detail-actions"><a class="action-btn" href="'+attr(window.routeHref?window.routeHref('races'): '#/races')+'" data-grimorio-route>← Todas as raças</a><div>'+((typeof window.shareLinkButton==='function')?window.shareLinkButton('race',race.id,forcedSubraceId?{subraceId:forcedSubraceId}:{},forcedSubraceId?'Copiar link da subraça':'Copiar link da raça'):'')+'<a class="action-btn" href="'+attr(window.routeHref?window.routeHref('ability-planner'):'#/ability-planner')+'" data-grimorio-route>Abrir Planejador de Atributos</a><button class="action-btn" onclick="GRIMORIO_RACE_BROWSER.clearChoices(\''+attr(race.id)+'\')">Limpar escolhas</button></div></div>'
      ...
      +'<div class="race-trait-grid legacy-grid">'+pool.map(x=>traitCard(x.trait,{selectable:true,selected:rs.legacy.includes(x.key),disabled:!rs.legacy.includes(x.key)&&!canSelectLegacy(race,rs.legacy,x.key),origin:x.origin,mixed:x.type==='mixed',onclick:'GRIMORIO_RACE_BROWSER.toggleLegacy(\''+attr(race.id)+'\',\''+attr(x.key)+'\')'})).join('')+'</div></section>'
