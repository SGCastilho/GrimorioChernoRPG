...
(function () {
  ...
  function subclassesTab(c, subs, selectedLevel) {
    ...
    return '<section class="class-section"><div class="section-heading"><div><h2 class="page-title small-title">Subclasses</h2><p class="section-help">' + subs.length + ' opções disponíveis para esta classe.</p></div></div>' +
      ...
      '<div class="subclass-card-grid">' + subs.map(s => {
        ...
        return '<a href="' + attr(routeHref('subclass',s.id,{classId:c.id})) + '" data-grimorio-route class="subclass-card ' + (unavailable ? 'unavailable-card' : '') + '" style="--card-color:' + attr(c.color) + '"><div class="subclass-card-top"><span class="mini-sigil" style="color:' + attr(c.color) + '">' + sigil(c.sigilKey || 'book') + '</span><div><h3>' + esc(s.name) + '</h3>' + sourceBadge(s.sourcePage, s.source?.title || c.source?.title || 'Livro do Jogador') + '</div></div><p>' + esc(cleanExcerpt(s.desc, 180)) + '</p><div class="subclass-card-meta">' + (unavailable ? 'Disponível a partir do nível ' + firstLevel : acquired + ' características ' + (selectedLevel ? 'disponíveis' : 'catalogadas')) + '</div><span class="read-link">Consultar subclasse →</span></a>';
      }).join('') + '</div></section>';
  }
  ...
  viewSubclass = function (id) {
    ...
    const parentCrumb = c ? routeAnchor('class',c.id,esc(parentName)) : '<span>' + esc(parentName) + '</span>';
    ...
    return '<div class="breadcrumb">' + routeAnchor('classes',null,'Classes') + '<span>/</span>' + parentCrumb + '<span>/</span><span>' + esc(s.name) + '</span></div>' +
