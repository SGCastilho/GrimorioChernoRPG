...
export async function hashPassword(password, salt = randomBytes(16)) {
  ...
  const derived = await scrypt(password, salt, SCRYPT.length, { N: SCRYPT.N, r: SCRYPT.r, p: SCRYPT.p, maxmem: 64 * 1024 * 1024 });
...
export async function verifyPassword(password, encoded) {
  try {
    ...
    const actual = await scrypt(password, Buffer.from(saltValue, 'base64url'), expected.length, { ...options, maxmem: 64 * 1024 * 1024 });
...
export function createSession(now = Date.now()) {
  ...
  const payload = { v: 1, iat: Math.floor(now / 1000), exp: Math.floor(now / 1000) + SESSION_SECONDS, nonce: randomBytes(18).toString('base64url'), av: authVersion(passwordHash) };
...
export function authenticate(request, now = Date.now()) {
  ...
  try {
    ...
    if (!body || !signature || extra !== undefined || !same(sign(body, sessionSecret), Buffer.from(signature, 'base64url'))) throw new Error('signature');
    const payload = JSON.parse(Buffer.from(body, 'base64url').toString('utf8'));
    if (payload.v !== 1 || payload.exp <= Math.floor(now / 1000) || payload.iat > Math.floor(now / 1000) + 60 || payload.av !== authVersion(passwordHash)) throw new Error('expired');
    return payload;
  } catch {
...
export function verifyCsrf(request, payload) {
  ...
  const expected = csrfToken(payload);
  const actualBuffer = Buffer.from(supplied, 'utf8');
  const expectedBuffer = Buffer.from(expected, 'utf8');
  if (!same(actualBuffer, expectedBuffer)) fail(403, 'INVALID_CSRF', 'A proteção da sessão não pôde ser validada.');
}
