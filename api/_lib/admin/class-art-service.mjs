...
function readState(snapshot) {
  ...
  if (!Array.isArray(manifest.classIndex) || manifest.classIndex.length !== manifest.classes) fail(500, 'INVALID_MANIFEST', 'O índice de classes não está configurado corretamente.');
  const classIds = new Set();
  for (const item of manifest.classIndex) {
    if (!item || typeof item.id !== 'string' || typeof item.name !== 'string' || classIds.has(item.id)) fail(500, 'INVALID_MANIFEST', 'O índice de classes contém dados inválidos.');
    classIds.add(item.id);
  }
  const parsed = {};
  for (const [kind, descriptor] of Object.entries(ART_FILES)) parsed[kind] = parseArtSource(snapshot.files[descriptor.path].content, descriptor.variable);
  for (const id of classIds) if (!parsed.cover.entries[id] || !parsed.detailArt.entries[id]) fail(500, 'INVALID_ART_SOURCE', `A classe ${id} não possui os dois mapas de arte.`);
  return { manifest, classIds, parsed };
}

function classView(item, parsed) {
  const cover = parsed.cover.entries[item.id];
  const detail = parsed.detailArt.entries[item.id];
  return {
    id: item.id,
    name: item.name,
    cover: effectiveEntry('cover', cover),
    detailArt: effectiveEntry('detailArt', detail),
...
export class ClassArtService {
  ...
  async list() {
    ...
    return {
      ...
      revisions: {
        ...
        coverFileSha: snapshot.files[ART_FILES.cover.path].sha,
        detailFileSha: snapshot.files[ART_FILES.detailArt.path].sha
      }
    };
  }
  ...
  async save(untrustedPayload) {
    ...
    for (const [kind, changes] of Object.entries(payload.changes)) {
      ...
      if (entryHash(currentEntry) !== payload.expected[expectedKey]) fail(409, 'CONFLICT', 'Esta classe foi alterada depois que o editor foi aberto. Recarregue os dados.');
    ...
    return {
      ...
      revisions: {
        ...
        coverFileSha: refreshedFiles[ART_FILES.cover.path].sha,
        detailFileSha: refreshedFiles[ART_FILES.detailArt.path].sha
      }
    };
  }
}
